...
pub fn open_file<P: AsRef<Path>>(path: P) -> Result<File, TmcError> {
    ...
    File::open(path).map_err(|e| FileIo::FileOpen(path.to_path_buf(), e).into())
}

pub fn read_file<P: AsRef<Path>>(path: P) -> Result<Vec<u8>, TmcError> {
    let path = path.as_ref();
...
pub fn create_file<P: AsRef<Path>>(path: P) -> Result<File, TmcError> {
    ...
    File::create(path).map_err(|e| FileIo::FileCreate(path.to_path_buf(), e).into())
}

pub fn remove_file<P: AsRef<Path>>(path: P) -> Result<(), TmcError> {
    let path = path.as_ref();
    fs::remove_file(path).map_err(|e| FileIo::FileRemove(path.to_path_buf(), e).into())
}
...
pub fn create_dir_all<P: AsRef<Path>>(path: P) -> Result<(), TmcError> {
    fs::create_dir_all(&path).map_err(|e| FileIo::DirCreate(path.as_ref().to_path_buf(), e).into())
}

pub fn remove_dir_empty<P: AsRef<Path>>(path: P) -> Result<(), TmcError> {
    fs::remove_dir(&path).map_err(|e| FileIo::DirRemove(path.as_ref().to_path_buf(), e).into())
}

pub fn rename<P: AsRef<Path>, Q: AsRef<Path>>(from: P, to: Q) -> Result<(), TmcError> {
    ...
    if source.is_file() {
        if target.is_dir() {
            ...
            let file_name = if let Some(file_name) = source.file_name() {
                ...
                return Err(TmcError::NoFileName(source.to_path_buf()));
            };
        ...
        if target.is_file() {
            return Err(TmcError::UnexpectedFile(target.to_path_buf()));
        } else {
