...
test_that("Test pass in simple_all_tests_pass", {
  ...
  test_results2 <- .run_tests_project(simple_all_tests_pass_project_path, addin_data)
...
test_that("Not all tests pass in simple_some_tests_fail.", {
  ...
  test_results2 <- .run_tests_project(simple_some_tests_fail_project_path, addin_data)
  test_results <- test_results2$test_results
...
test_that("run_results returns and writes.results.json as expected for simple_some_tests_fail", {
  ...
  expected_test_result[[1]] <- list(status      = "pass",
                                    name        = "ret_true works.",
                                    message     = "",
                                    backtrace   = list(),
                                    points      = list("r1", "r1.1"))
  expected_test_result[[2]] <- list(status      = "pass",
                                    name        = "ret_one works.",
                                    message     = "",
                                    backtrace   = list(),
                                    points      = list("r1", "r1.2"))
  expected_test_result[[3]] <- list(status      = "pass", 
                                    name        = "add works.",
                                    message     = "",
                                    backtrace   = list(),
                                    points      = list("r1", "r1.3", "r1.4"))
  ...
  # this might need a fix, since the backtrace is missing...
  expected_test_result[[4]] <- list(status      = "fail",
                                    name        = "ret_false returns true",
                                    message     = paste(sep = "\n",
                                                        "ret_false() is not TRUE", "",
                                                        "`actual`:   \033[32mFALSE\033[39m",
                                                        "`expected`: \033[32mTRUE\033[39m "),
                                    backtrace   = list(), #backtrace_test4,
                                    points      = list("r1", "r1.5"))
  expected_test_result[[5]] <- list(status      = "pass",
                                    name        = "ret_true works but there are no points.",
                                    message     = "",
                                    backtrace   = list(),
                                    points      = list("r1"))
  ...
  for (i in 1:5) {
    expect_equal(test_results_json[[i]], expected_test_result[[i]])
  }
...
test_that("Sourcing fail handled accordingly.", {
  ...
  expected_message   <- paste(sep = "\n",
                              ...
                              "7:   error in",
                              "           ^")
  expected_test_result <- list(list(status    = "fail",
                                    name      = "NA",
...
test_that("Test pass with overriden functions", {
  ...
  test_results2 <- .run_tests_project(simple_all_tests_pass_with_plot_project_path, addin_data)
  test_results <- test_results2$test_results
  # All tests should pass:
